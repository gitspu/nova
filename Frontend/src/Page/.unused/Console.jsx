...
import { H1, H2, H3, H4, H5, H6, P, Label, Button, Div, ModalHeader, Section, Span }   from '../Component/Common'
...
import { DivAbsolute } from "../Component/CommonEx";
...
const ViewDashboard = ({stateMenu}) =>
{
    ...
    */
    const Text = ({head = '' /** หัวข้อใหญ่ */, value = '' /** ค่าของข้อมูล */ }) =>
    {
        return (
          <Div>
            <P>{head}</P>
    ...
    const ChartLine = ({data = [], option = {} }) =>
    {
        return (
          <>
            <LineChart data={data} options={option}></LineChart>
          </>
        );
    }
    const ChartDoughnut = ({data = [], option = {} }) =>
    {
        return (
          <DoughnutChart data={data} options={option}></DoughnutChart>
        );
    }
    const ChartBar = ({data = [], option = {} }) =>
    {
        return (
          <BarChart data={data} options={option}></BarChart>
        )
    }
    ...
    return (
      <Div className={`dashboard ${menu == MENU_DASHBOARD ? 'd-block' : 'd-none'}`}>
        <ModalHeader>
          <H1 x-type='primary'>แดชบอร์ด</H1>
          <P x-type='secondary'>ยินดีต้อนรับสู่หน้าต่างแผงควบคุมระบบ</P>
        </ModalHeader>
        <Main>
          <Section>
            <H2>สถิติวันนี้</H2>
...
const ViewAuthentication = ({stateMenu}) =>
{
    ...
    return (
      <div className={`auth ${menu == MENU_AUTHENTICATION ? 'd-block' : 'd-none'}`}>
        ...
        <main>
          <Checkbox className='mb-2' state={[enableCreation, setEnableRegistration]} title='เปิดใช้งาน การสมัครบัญชี' subtitle='อนุญาตให้ผู้ที่ต้องการสามารถสมัครสมาชิกเพื่อเข้าถึงแพลตฟอร์มได้'/>
        ...
        const MenuProfile = () =>
        {
            return (
              <div className={menu == EDIT_MENU_PROFILE ? 'd-block' : 'd-none'}>
                <Settings.ProfilePersonal ref={profilePersonal}/>
                <Settings.ProfileContact ref={profileContact}/>
                <Settings.ProfileJob ref={profileJob}/>
                <Settings.ProfileEducation ref={profileEducation}/>
                <Settings.ProfileInterest ref={profileInterest}/>
                <Settings.ProfileSkill ref={profileSkill}/>
              </div>
            );
        }
...
const MainHeader = ({stateSelector, stateContext }) =>
{
    const [selector, setSelector] = stateSelector;
    const [context, setContext] = stateContext;

    /**
     * คำสั่งที่ถูกเรียกเมื่อต้องการออกจากระบบ
     */
    const onClickLogout = () =>
    {
        try     { api.auth.logout (); }
...
const MainBodyContent = ({menu}) =>
{
    ...
    return (
      <Root>
        <RootInner>
          <ViewDashboard stateMenu={menu}/>
          <ViewAuthentication stateMenu={menu}/>
          <ViewAccount stateMenu={menu}/>
          <ViewAdvertisement stateMenu={menu}/>
        </RootInner>
      </Root>
    );
}
const Main = () =>
{
    ...
    return (
      <>
        <MainBackground/>
        <MainBodyContent menu={[menu, setMenu]}/>
        <MainBodyMenu state={[menu, setMenu]}/>
        <MainHeader stateSelector={[selector, setSelector]} stateContext={[context, setContext]}/>
      </>
    )
}
export default Main;